...
half4 CalculateLight (v2f i)
{
	...
	// Alloy Code
	/////////////////////////////////////////////////////////////////////////////
	half3 e = normalize(_WorldSpaceCameraPos - wpos);
	half3 h = normalize(lightDir + e);
	half dotNL = saturate(dot(normal, lightDir));
	half dotNH = saturate(dot(normal, h));
	half d = AlloyBlinnPhong(dotNH, nspec.a);	
	
	// Set up specular lighting to use chromaticity light color recovery trick.
	half4 result;
	result.rgb = _LightColor.rgb;
  	result.a = AlloyLinearLuminance(_LightColor.rgb) * d; 
	
	// Dual-Lightmap fade.
	float fade = fadeDist * unity_LightmapFade.z + unity_LightmapFade.w;
	
	// Apply punctual lighting equation and dual lightmap fade.
	return result * (dotNL * atten * saturate(1.0h - fade));
}
