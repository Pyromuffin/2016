...
Shader "Hidden/Alloy/Nature/Terrain/Rsrm AddPass" {
Properties {
...
fixed4 frag_surf (v2f_surf IN) : COLOR {
  ...
  fixed4 lmtex = tex2D(unity_Lightmap, IN.lmap.xy);
  fixed3 lm = LightingAlloyBrdf_SingleLightmap(o, lmtex).rgb;
...
Pass {
		...
		Fog {Mode Off}
		Blend DstColor SrcColor, One One ZWrite Off
...
fixed4 frag_surf (v2f_surf IN) : COLOR {
  ...
  half3 lm = LightingAlloyBrdf_DualLightmap(o, lmtex, lmtex2, saturate(lmFade)).rgb;
  light.rgb += lm;
#else
  fixed4 lmtex = tex2D(unity_Lightmap, IN.lmap.xy);
  fixed4 lmIndTex = tex2D(unity_LightmapInd, IN.lmap.xy);
